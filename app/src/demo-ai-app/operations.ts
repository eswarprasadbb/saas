...
//#region Actions
export const generateGptResponse: GenerateGptResponse = async (args, context) => {
  const { hours } = args;
  if (!context.user) {
...
export const createTask: CreateTask = async (args, context: OperationContext) => {
...
export const updateTask: UpdateTask = async (args, context: OperationContext) => {
  const { id, isDone, time } = args;
  if (!context.user) {
    throw new HttpError(401);
  }
...
export const deleteTask: DeleteTask = async (args, context: OperationContext) => {
  const { id } = args;
  if (!context.user) {
    throw new HttpError(401);
  }
...
//#region Queries
export const getGptResponses: GetGptResponses = async (args, context: OperationContext) => {
  if (!context.user) {
    throw new HttpError(401);
  }
...
export const getAllTasksByUser: GetAllTasksByUser = async (args, context: OperationContext) => {
  if (!context.user) {
    throw new HttpError(401);
  }
